...
class Product extends CActiveRecord
{
  	    ...
  	    return array(
            'ImageUploadBehavior' => array(
                ...
                'images'=> array(
                    ...
                    'verylarge' => array('storage/product', 800, 600, 'prefix'=>'verylarge_'),
                ),
            ),
	...
	{
		$rules=array(
            array('covering, name', 'filter', 'filter'=>'strip_tags'),
            array('name, video, summary, description', 'filter', 'filter'=>'trim'),
            //array('summary, description','filter','filter'=>array($obj=new CHtmlPurifier(),'purify')),
			array('name, price, category_id, status, hit, novelty, shopwindow', 'required'),
			array('accessory, in_promotion, category_id, brand_id, status, hit, novelty, shopwindow, browse', 'numerical', 'integerOnly'=>true),
			...
			array('sname, article, number, variation, name, enterprise_1c_id', 'length', 'max'=>255),
			...
			array('id, name, covering, url, other_price, price, description, category_id, brand_id, status, hit, novelty, shopwindow, browse, priority', 'safe', 'on'=>'search'),
		);
    ...
    public function getFullnameDetail() {
        return $this->name.', '.$this->number.' / '.$this->article;
    }
	...
	public function attributeLabels()
	{
		return array(
            ...
            'video'=>'Видео с yotube',

            'in_promotion' =>'Участвует в акции',
    ...
    public function getCoveringList() {
        return array(
            'Золото'=>'Золото',
	...
	public function search()
	{
        ...
        if($this->query){
            $criteria->compare("CONCAT(t.name,' ',t.number,' ',t.sname,' ',t.article)",$this->query,true);
        }
        ...
        if($this->brand) {
            $ids=array($this->brand_id);
            foreach($this->brand->children as $ch) {
                $ids[]=$ch->id;
            }
            $criteria->compare('brand_id',$ids);
        }

		$criteria->compare('status',$this->status);
        $criteria->compare('covering',$this->covering);
        ...
        if($this->category_id=1 && empty($_GET['Product']['brand_id'])) {
             $criteria->join='LEFT JOIN {{brand}} as b ON t.brand_id=b.id';
             //$criteria->order='b.name ASC, t.price';
        }
