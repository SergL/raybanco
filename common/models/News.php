...
class News extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
            ...
            array('author_name, title', 'length', 'max'=>255),
            array('image', 'file', 'types'=>'jpg, gif, png, jpeg', 'allowEmpty'=>true),
            array('image', 'unsafe'),
            array('tags', 'length', 'max'=>1024),
            //array('tags', 'match', 'pattern'=>'/^[a-zA-Zа-яА-Я\_\s,0-9]+$/', 'message'=>'Теги могут содержать только символы и цифры.'),
            array('tags', 'normalizeTags'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('month,year, id, title, tags, status, publish_date, create_time, update_time', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
        ...
        if($this->month) {
            $criteria->compare('MONTH(publish_date)',$this->month);
        }

        if($this->year) {
            $criteria->compare('YEAR(publish_date)',$this->year);
        }
